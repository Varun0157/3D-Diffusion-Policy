...
def test_policy_chunked_mode_b(checkpoint_path, zarr_path, episode_idx=0, device='cuda', out_path='chunked_eval_mode_b.png'):
    ...
    # Mode B Tracking
    # We initialize the predicted positions with the first GT state
    # and then accumulate for the whole episode.
    history_b = [gt_state_data[0]] * n_obs_steps
    pred_positions_all[0] = gt_state_data[0]

    print(f"Running evaluation (Mode B - Accumulated): Episode {episode_idx} ({episode_len} steps), Chunk Size {chunk_size}")

    for step in tqdm(range(0, episode_len - chunk_size, chunk_size)):
        # 1. Prepare Observation (Mode B - use history)
        # obs_start/end not used directly for state anymore, we slice from history_b
        
        # Point cloud remains GT (or you could say environment is static/aligned)
        ...
        # Use history for agent_pos
        state_window = np.array(history_b[-n_obs_steps:])
        ...
        # 3. Integration & Storage (Mode B - NO GT RESET)
        # We continue from the LAST CALCULATED position in history_b
        curr_pos = history_b[-1].copy()
        
        for i in range(chunk_size):
            global_idx = step + i
            if global_idx >= episode_len: break
            ...
            next_pos[:6] += delta[:6]
            next_pos[6] += delta[6] # Gripper often absolute or thresholded
            
            if global_idx + 1 < episode_len:
                pred_positions_all[global_idx + 1] = next_pos
                history_b.append(next_pos)
    ...
    for i in range(num_dims):
        ...
        axes[i, 1].plot(pred_positions_all[:, i], label='Pred Pos (Accumulated)', color='#2ca02c', linewidth=1.5, linestyle='--')
        axes[i, 1].set_ylabel(f"{labels[i]} Pos")
        if i == 0: 
            axes[i, 1].set_title("Resulting Positions (Closed Loop - No GT Reset)")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--out', type=str, default="chunked_eval_mode_b.png")
    args = parser.parse_args()
    
    test_policy_chunked_mode_b(args.checkpoint, args.zarr_path, args.episode, out_path=args.out)
